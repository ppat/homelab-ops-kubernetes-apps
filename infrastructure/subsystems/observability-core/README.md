...
3. Required Variables
   ...
   | loki_s3_endpoint_key | Bitwarden key naming the S3 endpoint for Loki storage; defaults to `cluster_homelab_minio_loki_endpoint` if unset | Loki |
   | loki_s3_accesskeyid_key | Bitwarden key naming the S3 access key id for Loki storage; defaults to `cluster_homelab_minio_loki_accesskeyid` if unset | Loki |
   | loki_s3_secretkey_key | Bitwarden key naming the S3 secret access key for Loki storage; defaults to `cluster_homelab_minio_loki_secretkey` if unset | Loki |
