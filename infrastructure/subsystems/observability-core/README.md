...
- **An event carries its own timestamp, which bounds how much a restart can recover.** Entries are stamped
  with the event's `LastTimestamp`, not the ingest time, and Loki accepts an out-of-order entry only while it
  is within `ingester.max_chunk_age / 2` of the newest entry in that stream - 60 minutes at this module's
  settings. The singleton keeps its read watermark in a positions file on an `emptyDir`, so a restart makes
  it re-deliver whatever the API server still holds; that is bounded by the Kubernetes Event TTL of about an
  hour, i.e. it lands right at the edge of the acceptance window. A `PersistentVolumeClaim` would avoid the
  replay but would put a cluster-specific storage class inside the module, and a `hostPath` is meaningless
  for a pod that can move nodes - so the bounded duplicate burst is the accepted cost. Anything actually
  rejected is counted by `loki_discarded_samples_total`, and the `reason` label says which limit bit:
  `too_far_behind` is the `max_chunk_age / 2` window described above (`TooFarBehind` in Loki's
  `pkg/validation/validate.go`), and the lever for it is `ingester.max_chunk_age`, at the cost of chunks
  staying in ingester memory for longer. That value has not been observed on this cluster - the only `reason`
  the metric has ever carried here is `greater_than_max_sample_age`, which is a different limit
  (`reject_old_samples_max_age`, left at Loki's default of a week and therefore not what a ~1h event replay
  would ever trip). Check the label before reaching for the lever.
