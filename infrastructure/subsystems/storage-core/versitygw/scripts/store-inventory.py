...
# This predicate is `sweep.sh`'s, deliberately character-for-character -- the two
# must match or the residue series stops being the sweep's outcome. Change one
...
def render(state):
    ...
    add("# HELP versitygw_store_overwrite_residue_objects Stranded overwrite-race files beside their objects, per bucket.")
    ...
    add("# HELP versitygw_store_overwrite_residue_bytes Bytes held by stranded overwrite-race files, per bucket.")
