...
2. Required Variables
   ...
   | versitygw_data_claim | Name of the pre-provisioned PVC holding the object tree | VersityGW HelmRelease values, sweep CronJob, recovery-export Job, inventory exporter Deployment |
...
3. Storage Requirements
   ...
   | ${versitygw_data_claim} | VersityGW's object tree, plus its file-backed IAM store and its local-recovery kit as sibling subpaths | Provisioned externally by the cluster; not defined by this module |
...
- **The local-recovery kit lives on the object volume, in a `recovery` subpath, and it is refreshed by a `Job` named after the gateway version rather than by anything on a schedule.** The disaster-recovery path serves the tree with VersityGW run from a local image tarball, with no registry reachable, so the kit has to exist before the incident. The location does three jobs: a storage-side clone carries it, it sits outside the gateway root so it is neither enumerable as a bucket nor deletable over S3, and the `subPath` mount stops the exporting job reaching an object. The price of that placement, which is real and worth naming: the job inherits the gateway's node pinning, `fsGroup`, uid and affinity in order to write 150 MiB.
- **What has to be true of the kit is a fact about this directory in git, not about a registry, and the delivery mechanism follows from that.** The version that wrote the tree is a string in a YAML file here; a job polling a registry to discover whether that string moved is a runtime mechanism answering a question the repository had already answered, and the digest comparison, reuse branch and retention window that shape grew all existed to serve the inversion. A one-shot `Job` replaces it, re-run by Flux exactly when its image pins change. Three behaviours carry that, all measured against this estate's kustomize-controller rather than assumed: `spec.template` is **immutable**, so a changed `Job` is rejected with `field is immutable`; the `kustomize.toolkit.fluxcd.io/force: enabled` annotation makes Flux recognise *that specific error* and delete then recreate the object, so a pin bump re-runs the export; and because force applies only on an immutable-field error, an identical apply stays an ordinary no-op, so a completed `Job` is reconciled over indefinitely without re-pulling. The version therefore lives in exactly **one** place on this path and a gateway bump stays an ordinary automergeable update that also refreshes the kit. **The `Job` carries no `ttlSecondsAfterFinished`**: the TTL controller would delete the completed object, Flux would recreate it from the same desired state, and the export would loop. **The invariant to hold it to: every `versity/versitygw` version in this directory names the same build** -- all three references are Renovate-tracked for the same dependency, so they move in one update, and a kit holding a version other than the one that wrote the tree reintroduces the failure it exists to prevent.
- **The kit ships a written procedure, not only instruments.** `RECOVERY-RUNBOOK.md` is exported onto the volume beside the tarballs and the check script, because the scripts cite it for the values that arm their checks -- the expected `store_id`, the original LUN's WWID, the bucket names -- and a procedure that exists only in this repository is unreachable on a host with no registry, no internet and no Kubernetes, which is precisely the host the path is designed for. It also carries the steps no script performs: the DSM clone, the iSCSI attach, and the `versitygw` invocation itself. Two findings live there rather than only in code because an operator has to see them: `mount -o ro` on an unreplayed ext4 journal **writes to the device**, so there is no safe read-only look at production; and a clone carries the same filesystem label and UUID as the original, so the WWID is the only device-level discriminator -- a **match** against the recorded one means you are on production. The kit ships an S3 client beside the gateway because restoring over S3 needs one and on a host with no registry it is equally unavailable; it arrives as a docker image rather than a binary, which is why the check script takes the client's invocation as a parameter instead of calling a bare `aws`.
