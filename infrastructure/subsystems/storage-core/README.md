...
5. Backup Object Storage (VersityGW)
   ...
   - Prometheus metrics from three producers deployed alongside, since the gateway itself publishes only UDP counters: a StatsD translator for the traffic it serves, a read-only inventory exporter for what the store actually holds, and a WAL-chain continuity reader for whether an archived PostgreSQL WAL sequence in the store still has every segment it should
...
2. Required Variables
   ...
   | versitygw_data_claim | Name of the pre-provisioned PVC holding the object tree | VersityGW HelmRelease values, sweep CronJob, inventory exporter Deployment, WAL continuity reader Deployment |
...
- **The store's contents are exported by a walk, because nothing else can produce them.** The gateway's counters describe traffic, not holdings, and free space reads identically for a store that is empty because it is new and one that is empty because the volume beneath it was reformatted — which the in-tree iSCSI plugin does silently. A resident exporter therefore walks the gateway root read-only and publishes per-bucket object counts and byte totals, with the walk's completion timestamp beside them. Four properties are load-bearing. It mounts the **volume root read-only** and walks the `data` subdirectory rather than mounting `subPath: data`, because the kubelet *creates* a missing `subPath` — so an absent gateway root makes it refuse to start rather than manufacture one and report a store with nothing in it. It declares **no `fsGroup`**, since it only reads and the tree already carries the uid it runs as; adding one re-opens the mismatch hazard in the note above. Its interval is a **floor rather than a schedule** — the walk sleeps after finishing, so a walk slower than the interval degrades cadence instead of stacking load, and a rollout restart forces one immediately. A walk that *raises* does not wait out that interval: one unreadable directory unwinds the whole pass, and at a daily cadence sleeping it off would cost a day of currency, so failures retry on a doubling backoff that settles back to the interval. And its script is generated into its own **content-hashed** `ConfigMap`, unlike the shared one, because it is the only consumer here that reads its script once and holds it. The walk also splits residue the way the sweep does, and the coupling is deliberate: the overwrite-race predicate in `store-inventory.py` is `sweep.sh`'s own, character-for-character, because a residue series that classifies differently from the sweep stops being a reading of the sweep's outcome. Those files are counted in **both** the object totals and the overwrite-residue totals -- they are S3-visible as ordinary keys, so excluding them would break the exported-count-equals-`ListObjectsV2` property, while excluding them from residue is how unreclaimable bytes accumulate with the object count rising and residue flat, both reading reassuring.
- **Two things to know before writing a query against any of these series.** A labelled check of the form `versitygw_store_objects{bucket="x"} < N` returns an *empty* result when that bucket is deleted outright, which reads as "nothing wrong" — the form that fires is `(… or vector(0)) < N`, and the same hazard applies to every per-server WAL series, since a server's series vanish with its directory. And a zero is equally consistent with a healthy store and a dead producer, so each producer's numbers are only interpretable alongside its own completion timestamp — `versitygw_store_last_scan_timestamp_seconds` and `versitygw_wal_archive_last_scan_timestamp_seconds` — which is why both are exported. `versitygw_store_buckets` and `versitygw_wal_archive_servers` are the two series that stay present through a completed pass whatever it found, and are therefore the anchors that separate "nothing wrong" from "nothing there".
- **The gateway never calls `fsync`.** An acknowledged write becomes durable at the next filesystem journal commit, not at acknowledgement, so a crash inside that window loses a completed, acknowledged object. Nothing in this module closes that window — narrowing it is a mount-option and power-protection concern belonging to the cluster that provides the volume. What this module does provide is the instrument that makes a loss inside it *visible*: PostgreSQL recycles a WAL segment once archiving is acknowledged, so from that moment the store can hold the only copy, and the continuity reader below turns the resulting hole into a number rather than a surprise during a restore.
- **WAL-chain continuity is read from the store, because every other view of it is a belief rather than an observation.** Everything CloudNativePG exports about archiving derives from `pg_stat_archiver` — Postgres recording that its archive command exited 0, which is the claim the un-`fsync`ed window falsifies — and the barman-cloud plugin's `ObjectStore` status reads the store only at base-backup granularity, so it cannot see a hole open between two backups. A reader of the tree is the only thing that can. It discovers archives by shape (`<bucket>/<prefix>/<server>/wals/`), so it names no bucket, database or cluster generation, and reports per server and timeline: segments held, segments **missing between the first and last held**, distinct gaps, zero-length segments, and missing timeline history files. Three things about it are load-bearing. Contiguity is measured only across the interval actually held, because an archive legitimately begins where its consumer started writing to *this* store — a chain measured from zero would be red for every server forever. Zero length is checked and content is **not**: an emptied object is unambiguously damaged at any compression setting, whereas verifying content means reading the entire archive back on every pass, against an at-rest integrity story that belongs to the array's redundancy and scrub. And a lost **tail** moves no gap count at all, since nothing brackets it — which is why `versitygw_wal_archive_last_segment_ordinal` is exported: it is `LSN >> 24` at PostgreSQL's default segment size, and therefore comparable with what Postgres believes it archived. **A green missing-segment count does not on its own establish that a chain is whole.** Two more, both about series that would otherwise be silently absent rather than wrong. A history file is expected for every timeline held **from the lowest upward**, excluding only timeline 1 -- bounding it at *lowest + 1* instead reads every archive as if it began on timeline 1, which almost none of them do here: `components/db-restore` rotates generations, and a restored generation archives under a new `serverName` starting on the timeline it was promoted to, so its own `.history` file is both the first thing it writes and the one whose loss makes it unrecoverable with every segment present. And a server whose `wals/` survives while its contents do not holds no chain, so it would contribute no per-timeline row to anything and simply stop being reported -- it therefore gets a segment count of 0 and a newest-segment timestamp of 0 emitted **without** a timeline label, because a rule-side `or vector()` substitutes for an expression that is empty overall and never for one server's missing row among others still reporting.
- **A gap at a migration cutover is a true reading, not a false alarm.** Producers keep writing to the old store until their own repoint, so unless the delta written between the copy and the cutover is copied too, the new store's chain really does have a hole spanning that interval. The metric going red at the cutover and green again after the delta copy is the check that the seam was closed — it should not be suppressed for the migration it exists to watch.
