...
| **Version Management** | Renovate Bot via Github Actions | Automated version upgrades | - Scans for dependency updates across and within modules, including the chainsaw fixtures that pin the same images<br>- Auto-merges routine updates once the module's own test suite passes<br>- Routes updates that carry a compatibility claim to human review, judged by the versioning scheme the dependency actually uses |
