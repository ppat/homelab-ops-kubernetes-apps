...
| `sso` | — | patches `HelmRelease`/`Ingress` across seven modules by name+namespace. A target matching nothing is a **silent no-op**, so the failure is SSO switched off rather than a build error — only a suite deploying the target module *with* the component and asserting the patched values can see it |
| `cert-issuer/letsencrypt` | — | ACME `ClusterIssuer`s with a DNS-01 solver; needs an ACME account and a delegable zone |
