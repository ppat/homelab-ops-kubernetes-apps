...
| --- | --- | --- |
| AI | Language Models<br/>Chat Interfaces<br/>AI Gateway<br/>Workflow Automation | [ai](./subsystems/ai):<br/>• Provides web-based chat via <a href="https://github.com/open-webui/open-webui" target="_blank">OpenWebUI</a>, backed by a self-hosted LiteLLM gateway<br/>• Routes cloud model traffic through a self-hosted <a href="https://github.com/BerriAI/litellm" target="_blank">LiteLLM</a> gateway with automatic provider failover, virtual key/budget management, and request caching<br/>• Exposes self-hosted MCP servers for documentation lookup, browser automation, observability, Kubernetes, network, home automation, and GitHub to any gateway client<br/>• Supports conversation history management<br/>• Automates glue workflows via a self-hosted <a href="https://n8n.io" target="_blank">n8n</a> instance (own namespace/Postgres) — Alertmanager, *arr, and email integrations<br/>• Provides a WhatsApp-reachable conversational front door via <a href="https://github.com/openclaw/openclaw" target="_blank">OpenClaw</a> (own namespace, hardened), able to trigger n8n workflows and relay results back |
| Knowledge Management | Knowledge Vault<br/>Vault Write Path<br/>Durable Work Queue | [obsidian-vault](./subsystems/obsidian-vault):<br/>• Runs a git-backed markdown knowledge vault on a headless in-cluster <a href="https://obsidian.md" target="_blank">Obsidian</a>, the only process that ever mutates a vault file<br/>• Fronts it with two separately-scoped MCP server instances — the only network path to vault content — reachable from the LiteLLM gateway alone<br/>• Provides a durable, ordered, acknowledged write path over <a href="https://nats.io/" target="_blank">NATS JetStream</a>, reachable off-cluster over TLS for producers outside the cluster<br/>• Drains bulk work through the same gated write path as interactive ingest, never around it<br/>• Commits the vault's history to a GitHub remote on a schedule, mounting vault content read-only<br/>• Publishes an independent vault-loaded signal that can contradict the editor pod's own readiness |
...
flowchart BT
    ...
    subgraph Apps["Application Modules"]
        ...
        ai[ai]:::apps
        obsidian-vault[obsidian-vault]:::apps
