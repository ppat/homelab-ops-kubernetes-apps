...
flowchart TB
    ...
    subgraph module[AI Module]
        ...
        batchprocessor[Batch Processor<br/>CronJob + stream provisioner + watchdog]:::core
        ...
        subgraph vault[Knowledge Vault — ns: obsidian-vault]
            ...
            vaultexporter[Vault-Loaded Exporter<br/>independent observer]:::core
        end
    end
    ...
    vaultexporter -->|enumerates vault content| obsidian
    ...
    batchprocessor -.->|drain the batch stream, scheduled| obsidian_vault_nats
    batchprocessor --> litellm
```
...
3. Required Variables
   ...
   | ---------- | --------- | --------- |
   | domain_name | External access URL (openwebui.${domain_name}, obsidian-vault-nats.${domain_name}) | OpenWebUI, obsidian-vault-nats, batch-processor |
...
- **`app.kubernetes.io/component: mcp-server` pod label**: every MCP server Deployment's pod template (not its immutable `spec.selector`) carries this label, giving every MCP server a single stable selector the `mcp-servers-ingress` NetworkPolicy targets regardless of `app.kubernetes.io/name`. It exists so a server added later by copying an existing module directory is covered by that policy by default rather than silently unprotected. The two Obsidian MCP servers carry the same label in the `obsidian-vault` namespace, where it selects both the policy admitting LiteLLM inbound and the policy admitting them onward to Obsidian. One workload carries it without speaking MCP at all — see the vault-loaded exporter note below — so read the label as "is admitted by the MCP ingress exceptions", not as "implements the MCP protocol".

- **The vault-loaded exporter carries `app.kubernetes.io/component: mcp-server` and is not an MCP server.** It is an independent observer of whether headless Obsidian can actually open the vault: an authenticated call enumerating vault content, a non-empty assertion, a gauge and a last-success timestamp. The label is reuse for NetworkPolicy admission and nothing else — it is the selector `obsidian-ingress` evaluates to let this pod reach Obsidian, and the selector `mcp-obsidian-ingress` evaluates to let Prometheus scrape it, which together are why this workload lands with zero policy changes. Widening either policy for it was never on the table: `obsidian-ingress` is the sole control described above. Two things follow that are easy to get wrong later. It must not be given a readiness or liveness path onto the vault signal, and nothing may act on that signal by restarting anything — the failure it exists to catch left the editor pod `1/1 Ready` with its REST API serving, and a restart does not fix it, so a probe would have produced a crashloop instead of a signal. And it holds the same write-anywhere REST API bearer token the MCP servers do, because that listener has exactly one token; a read-only observer with a narrower credential is not available to be built. See `ppat/obsidian-tools` ADR-0037.
