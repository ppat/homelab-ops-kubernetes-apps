...
2. Required Secrets
   ...
   | kubeconfig_sandbox_mcp | **Provisional** — kubeconfig for the sandbox Talos cluster, mounted into the self-hosted mcp-kubernetes-sandbox server so it can reach (and write to) that cluster's API. Depends on a credential relay (ppat/homelab-ops-kubernetes-experiments#225) that has not landed yet; the key name/shape may still change |
