...
3. Work Queue Substrate
   ...
   - A watchdog starts the agent-facing MCP server again when the run holding it stopped stops renewing its lease
...
flowchart TB
    ...
    kubeapi[Kubernetes API]:::external
    ...
    batch -->|scale to 0, take the lease| kubeapi
    watchdog -->|scale to 1 on an expired lease| kubeapi
    kubeapi -.->|replicas| mcpagent
...
2. Required Secrets
   ...
   | batch-processor-secrets | Everything one batch run presents: the broker credential permitted to consume, and a LiteLLM virtual key scoped to the ingestor MCP server | BATCH_PROCESSOR_NATS_PASSWORD, BATCH_MCP_API_KEY |
...
- **`mcp-obsidian-agent` declares no `replicas`, and that omission is a control.** The replica count is runtime state owned by batch mode: a run scales the Deployment to zero for its duration and back to one at the end (ppat/obsidian-tools ADR-0052). Under server-side apply a field absent from the applied manifest is not owned by the applier, so a reconcile during a live run leaves the zero in place; declaring `replicas: 1` would restore the pod mid-run and let interactive writers race the batch through the editor's single event loop. The failure is silent in the dangerous direction — writes work, nothing looks broken, and the invariant the whole mechanism exists to hold is gone. A HorizontalPodAutoscaler pointed at this Deployment breaks it identically. Note that the *transition* apply, the one that first drops the field, does reset the count to the API server's default of one, so it should not be reconciled while a run is in flight.

- **The batch-mode grant's narrowness is the reason the mechanism was chosen, not hygiene applied afterwards.** The rejected alternative — refusing the agent server at the network for a run's duration — required authority to edit `NetworkPolicy` in this namespace, where `obsidian-rest-api-ingress` is the *sole* control in front of an undisableable, unscoped write endpoint. A workload holding that authority can open the whole vault. So the replacement grant (`batch-processor/rbac.yaml`) is `get`/`patch` on `deployments/scale` with `resourceNames: [mcp-obsidian-agent]`, and `get`/`patch` on one named `Lease`. Four things in it are load-bearing: the `scale` subresource rather than the Deployment (whose body carries `OBSIDIAN_WRITE_PATHS` and the image); `resourceNames` excluding `obsidian` and `mcp-obsidian-ingestor`, so a run can neither stop the only writer of vault content nor cut its own write path; **no `create` verb anywhere**, because RBAC cannot restrict `create` by `resourceNames` — which is why the `Lease` ships in git and nothing can recreate it if deleted; and `patch` rather than `update`, so a write can never carry away a field it did not send. The chainsaw suite asserts every one of these, but a Role definition is a claim about intent — only the API server refusing a scale of the ingestor instance is evidence, and that injection is owed on a real cluster.
