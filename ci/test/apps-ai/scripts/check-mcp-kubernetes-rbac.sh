...
check_sandbox_write_boundary() {
  ...
  # "context deadline exceeded" tool error is a REPLY, so the cap must sit clear of it rather
  # than cutting off a call that did in fact answer.
  #
  # What that reply is then classified AS is a separate, open defect -- NOT something the
  # branches below already handle. It carries "isError":true without the pinned denial, so it
  # reaches the final branch and is reported as the control having failed OPEN, on a run where
  # nothing was read at all. See #3899.
