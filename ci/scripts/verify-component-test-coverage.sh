...
declare -A UNCOVERED=(
  # sso and cert-issuer are uncovered by any SUITE, but not unchecked: their patch targets are
  # verified statically by verify-component-patch-targets.sh, which covers all ten of sso's at
  # once. A suite could only ever reach the one or two whose module it deploys, and for sso it
  # cannot reach even that -- see the measurement below.
  ["components/sso"]="Patches HelmReleases and Ingresses across seven modules by name+namespace. Its whole silent surface (a target matching nothing) is covered by verify-component-patch-targets.sh; what a suite would add is proof the patched output is accepted by a live API server. apps-coder cannot host it: Coder performs OIDC discovery at startup and exits 1 when the issuer is unreachable (measured on v2.35.7 against a real Postgres: 'create oidc config: configure oidc provider: Get .../.well-known/openid-configuration: no such host'), so the suite would need a TLS-serving stub IdP, not just a spec.components line."
  ["components/cert-issuer/letsencrypt"]="Its one patch target is covered by verify-component-patch-targets.sh. The ClusterIssuers it creates are not: exercising them needs an ACME account and a delegable DNS zone, which would make the suite depend on Let's Encrypt staging and on a real zone."
